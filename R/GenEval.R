...
#' \item{FixedEff}{The estimated fixed effects.}
#' \item{VarComp}{The estimated variance components for all random effects, and for the residual.}
#' \item{RandomEff}{The predicted random effects and their reliabilities.}
#' \item{modelFit}{A summary of the fit from the linear mixed model.}
#' \item{modelPred}{A summary of the prediction from the linear mixed model (when TestGroup is provided).}
#' }
#' @examples ## SINGLE COMPONENT WITH FIXED EFFECTS ##
#' @examples # simulate genotypes
#' @examples M <- simGeno(n=500,AlleleFreq=runif(100,0.05,0.5))
...
#' @examples str(aux)
#' @examples # prediction
#' @examples Vlist <- list(diag(1,70),diag(1,30))
#' @examples Zlist <- list(M[,1:70],M[,71:100])
#' @examples aux <- BLUPsolve(y=pheno$y,VarMat=Vlist,Z=Zlist,TestGroup=401:500)
#' @examples str(aux)
